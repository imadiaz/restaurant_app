...
export const menuSectionService = {
  ...
  async getAllByRestaurantId(restaurantId: string): Promise<MenuSection[]> {
    const response = await axiosClient.get<unknown, ApiResponse<MenuSection[]>>(`/menu-sections/restaurant/${restaurantId}`);
    return response.data;
  },

  // Get a single section by ID
  async getById(id: string): Promise<MenuSection> {
    const response = await axiosClient.get<unknown, ApiResponse<MenuSection>>(`/menu-sections/${id}`);
    return response.data;
  },

  // Create a new section
  async create(data: CreateMenuSectionDto): Promise<MenuSection> {
    const response = await axiosClient.post<unknown, ApiResponse<MenuSection>>('/menu-sections', data);
    return response.data;
  },

  // Update a section (Name, Order, or Status)
  async update(id: string, data: UpdateMenuSectionDto): Promise<MenuSection> {
    const response = await axiosClient.patch<unknown, ApiResponse<MenuSection>>(`/menu-sections/${id}`, data);
    return response.data;
  },

  async toggleStatus(id: string, data: UpdateMenuSectionStatus): Promise<MenuSection> {
    const response = await axiosClient.patch<unknown, ApiResponse<MenuSection>>(`/menu-sections/${id}/status`, data);
    return response.data;
  },

  // Delete (Optional, good to have)
  async delete(id: string): Promise<void> {
    await axiosClient.delete<unknown, ApiResponse<void>>(`/menu-sections/${id}`);
  }
};