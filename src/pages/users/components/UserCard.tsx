...
<div className="flex justify-center mt-1">
  ...
  `}>
    <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${user.status === STATUS.active? 'bg-green-500' : 'bg-red-500'}`}></span>
    {user.status}
  </span>
</div>
          ...
          <AnatomyButton
            ...
            onClick={onClick}
          >
            {t('common.details')} <ExternalLink className="w-4 h-4 ml-2" />
          </AnatomyButton>
        </div>
      </div>
    </div>
