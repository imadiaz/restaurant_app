...
import { Save, CircleDot, CheckSquare, GripVertical, Package, Trash2, Type, PackageSearch } from 'lucide-react';
...
import type { CreateModifierOption, CreateModifierGroup } from '../../service/products.service';
...
const ModifierFormPage: React.FC = () => {
  ...
  const { createGroup, updateGroup, modifiers, isLoading: isListLoading, isCreating, isUpdating } = useModifiers();
  ...
  const updateOption = (idx: number, field: keyof CreateModifierOption, val: any) => {
  ...
  const handleSave = async () => {
    ...
    for (const opt of options) {
      ...
      if (Number(opt.price) < 0) {
        addToast(t('modifiers.error_option_price',{name: opt.price}), "error");
        return;
      }
  ...
  return (
    <BasePageLayout
      title={isEditMode ? t('products.edit_modifier') : t('products.add_modifier')}
      ...
      <div className="max-w-4xl mx-auto pb-20 space-y-6">
        ...
        <div className="bg-background-card p-6 rounded-3xl border border-border shadow-sm space-y-4">
           <div className="flex justify-between items-center border-b border-border pb-4">
              ...
              <AnatomyText.Small className="text-text-muted">{options.length} options</AnatomyText.Small>
           </div>
              ...
              <Droppable droppableId="opts">
                 {(provided) => (
                    <div ref={provided.innerRef} {...provided.droppableProps} className="space-y-2">
                       {options.map((opt, idx) => (
                          <Draggable key={opt.id || idx} draggableId={opt.id || String(idx)} index={idx}>
                             {(provided) => (
                                <div
                                ...
                                >
                                   <div {...provided.dragHandleProps} className="text-text-muted cursor-grab active:cursor-grabbing"><GripVertical className="w-4 h-4" /></div>
                                   ...
                                   <div className="flex items-center justify-center w-8 h-8 shrink-0 mr-2">
                                      {opt.productId ? (
                                          ...
                                          <div className="text-gray-300">
                                            {isSingle ? <CircleDot className="w-5 h-5" /> : <CheckSquare className="w-5 h-5" />}
                                          </div>
                                      )}
                                   </div>
                                   ...
                                   <div className="w-16 border-l border-border pl-2 flex flex-col justify-center">
                                      <div className="flex items-center gap-1">
                                         <span className="text-text-muted text-[10px] font-bold">x</span>
                                         <input 
                                            type="number" 
                                            min="1"
                                            ...
                                            onChange={(e) => updateOption(idx, 'maxQuantity', Number(e.target.value))}
                                         />
                                      </div>
                                      <AnatomyText.Label className="text-[9px] text-text-muted leading-none mt-0.5">{t("products.max_quantity")}</AnatomyText.Label>
                                   </div>
                                   ...
                                   <div className="w-24 border-l border-border pl-2 flex flex-col justify-center">
                                      ...
                                      <AnatomyText.Label className="text-[9px] text-text-muted text-right leading-none mt-0.5">{t("products.extra_price")}</AnatomyText.Label>
                                   </div>
           ...
           <div className="flex gap-2 pt-2">
              ...
              >
                 <PackageSearch className="w-3 h-3 mr-2" /> {t("products.add_product_option")}
              </button>
           </div>
        </div>
      </div>
