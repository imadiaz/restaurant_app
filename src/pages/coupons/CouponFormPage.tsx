...
import { Save, AlertCircle, Calendar, DollarSign, Percent, Tag, Users, Hash } from 'lucide-react';
...
import { type CreateCouponDto, CouponScope, CouponType } from '../../service/coupons.service';
...
const CouponFormPage: React.FC = () => {
  ...
  // Hooks
  const { createCoupon, updateCoupon, getCouponById, isCreating, isUpdating, isLoading: isLoadingList } = useCoupons();
 ...
 const hasLoadedData = useRef(false); 
  ...
  const onSubmit = async (data: CreateCouponDto) => {
    ...
    try {
      const payload: any = {
        ...
        usageLimitGlobal: Number(data.usageLimitGlobal) || null,
  ...
  return (
    <BasePageLayout
      ...
      subtitle={isEditMode ? t('coupons.edit_description') : t('coupons.add_description')}
      showNavBack={true}
      headerActions={
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
           ...
           <div className="lg:col-span-2 space-y-6">
              <div className="bg-background-card p-6 rounded-3xl shadow-sm border border-border space-y-6">
                 <AnatomyText.H3 className="text-lg mb-4">{t('coupons.section_general')}</AnatomyText.H3>
              ...
              {/* Rules Section */}
              <div className="bg-background-card p-6 rounded-3xl shadow-sm border border-border space-y-6">
                  <AnatomyText.H3 className="text-lg mb-4">{t('coupons.section_rules')}</AnatomyText.H3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        <AnatomySelect label={t('coupons.discount_type')}>
                           <option value={CouponType.PERCENTAGE}>{t('coupons.type_percentage')}</option>
                              <option value={CouponType.FIXED_AMOUNT}>{t('coupons.type_fixed')}</option>
                              <option value={CouponType.FREE_DELIVERY}>{t('coupons.type_free_shipping')}</option>
                        </AnatomySelect>
                      ...
                      <AnatomyTextField 
                        ...
                        icon={couponType === CouponType.PERCENTAGE ? <Percent className="w-4 h-4"/> : <DollarSign className="w-4 h-4"/>}
                        placeholder="0.00"
           ...
           <div className="space-y-6">
              ...
              <div className="bg-background-card p-6 rounded-3xl shadow-sm border border-border space-y-4">
                 <AnatomyText.H3 className="text-base font-medium">{t('coupons.section_validity')}</AnatomyText.H3>
              ...
              {/* Limits */}
              <div className="bg-background-card p-6 rounded-3xl shadow-sm border border-border space-y-4">
                 <AnatomyText.H3 className="text-base font-medium">{t('coupons.section_limits')}</AnatomyText.H3>
